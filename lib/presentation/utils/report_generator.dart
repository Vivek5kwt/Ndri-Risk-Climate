...
class ReportGenerator {
  ...
  static String _asFixed(dynamic val, {int digits = 3}) {
    ...
    if (val is String) return (double.tryParse(val) ?? 0).toStringAsFixed(digits);
    return '0.${'0'*digits}';
  }
  ...
  async {
    ...
    final double acceptedExposure      = _clamp01(_normalize(rawExp,    _expMin, _expMax));
    final double acceptedHazard        = _clamp01(_normalize(rawHazard, _hazMin, _hazMax));
    final double acceptedVulnerability = _clamp01(_normalize(rawVuln,   _vulnMin, _vulnMax));
    ...
    final double calculatedRisk = acceptedExposure * acceptedHazard * acceptedVulnerability;
    final double acceptedRisk   = calculatedRisk / _riskDivisor; // no clamp (per Excel)
    ...
    top: centerY - dotSize - 88, // tuned for your asset
    ...
    height: PdfPageFormat.a4.height * 1.4, // ~2.6x taller than A4 to fit everything
    marginLeft: 0,
    marginRight: 0,
    marginTop: 0,
    marginBottom: 0,
    ...
    name,
    style: pw.TextStyle(
    fontSize: 22, color: PdfColor.fromHex('#0303ff'), font: garamondSemiBold),
    ),
    ),
    ]),
    ...
    pw.Text(
    'State: ',
    style: pw.TextStyle(fontSize: 22, color: stateBlockLabelColor, font: garamondBold),
    ),
    pw.Expanded(
    child: pw.Text(
    stateName ?? '',
    ...
    pw.Text('District: ',
    style: pw.TextStyle(
    fontSize: 22, color: stateBlockLabelColor, font: garamondBold)),
    pw.Expanded(
    child: pw.Text(
    district ?? '',
    style: pw.TextStyle(
    ...
    centerYOffset: 78,   // tweak pointer center for larger height
    ...
    pw.SizedBox(height: 6),
    pw.Text(
    _asFixed(acceptedRisk, digits: 2), // show up to two digits
    style: pw.TextStyle(
    fontSize: 36, // a bit larger to match bigger meter
    fontWeight: pw.FontWeight.bold,
    ...
    style: pw.TextStyle(fontSize: 20, font: garamondBold)),
    ...
    pw.Padding(
    padding: const pw.EdgeInsets.only(top: 12, left: 5, right: 8),
    child: pw.Column(
    crossAxisAlignment: pw.CrossAxisAlignment.start,
    children: [
    pw.Text('Remarks:',
    style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold, font: garamondBold,color: stateBlockLabelColor)),
    pw.Text(
    ...
    style: pw.TextStyle(fontSize: 16, font: garamondMedium),
    ),
    ],
    ),
    ),
    ...
    children: [
    _legendRow(PdfColor.fromHex('#1d4517'), 'Very Low'),
    _legendRow(PdfColor.fromHex('#03cd03'), 'Low'),
    _legendRow(PdfColor.fromHex('#0303ff'), 'Moderate'),
    _legendRow(PdfColor.fromHex('#ff0303'), 'High'),
    _legendRow(PdfColor.fromHex('#c10303'), 'Very High'),
    ],
    ),
    ),
    ...
    pw.Text(
    'Disclaimer: ',
    style: pw.TextStyle(fontSize: 18, color: disclaimerColor, font: garamondExtraBold),
    ),
    pw.Expanded(
    child: pw.Text(
